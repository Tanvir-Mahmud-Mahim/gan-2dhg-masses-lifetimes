...
def coupling_matrices(bands, w_fn, eps_r, b_fh, overlap=None, screen=True,
    ...
    unity on the diagonal.  Passed as a constant matrix it is the crude
    treatment: the overlap in fact depends on the angle between the initial and
    final wavevectors, and it belongs inside the angular integral, not outside
    it.

    overlap_fn(i, j, theta), if given, supersedes overlap and is evaluated at
    every angle.  The envelope-function solver computes exactly this quantity
    from the six-component spinors at the two Fermi circles, so with
    overlap_fn supplied there is no free parameter left in this calculation.
    """
    n = len(bands)
    ...
    for i in range(n):
        for j in range(n):
            ...
            pref = mj / (2.0 * PI * HBAR**3)
            const = 1.0 if overlap_fn is not None else overlap[i][j]
            ...
            def integ(theta, weight):
                ...
                if overlap_fn is not None:
                    w = w * overlap_fn(i, j, theta)
                return w * (np.cos(theta) if weight else 1.0)

            A[i, j] = pref * const * quad(integ, 1e-6, 2.0 * PI - 1e-6,
                                          args=(False,), limit=200)[0]
            B[i, j] = pref * const * quad(integ, 1e-6, 2.0 * PI - 1e-6,
